  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.cityContainer}>
        {error && <Text style={styles.error} variant="bodyLarge">{error}</Text>}
        {loading && <ActivityIndicator size="large" animating={true} color={paperTheme.colors.fontPrimary} />}
        <FlatList
          data={cityList}
          showsVerticalScrollIndicator={false}
          // renderItem={({ item }) => <UserListItem key={item.email} type={EUserListItem.add} user={item} />}
          renderItem={({ item }) => <Text style={{ color: 'white'}} variant="headlineMedium">{item.country}</Text>}
        />
      </View>
